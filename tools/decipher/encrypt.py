...
def encrypt_input(input_text: str, encryption_key: bytes) -> str:
    ...
    return ''.join([chr(b + ord("一")) for b in iv]) + ''.join([chr(b + ord("一")) for b in encrypted])

def main() -> None:
    """Main function to handle command-line arguments and decryption."""
    parser = argparse.ArgumentParser(description="Encrypt a cipher text with AES and CBC mode.")
