...
void SplitInput::initLayout()
{
    ...
    this->signalHolder_.managedConnect(this->split_->channelChanged, [this] {
        this->updateEncryptionState();
    });

    getSettings()->encryptionChannelStates.connect(
        [this](const QHash<QString, bool> &, auto) {
            this->updateEncryptionState();
        },
        this->managedConnections_);
    ...
    QObject::connect(this->ui_.encryptionToggleCheckbox, &QCheckBox::toggled,
                     this, [this](bool value) {
                         const auto channel = this->messageChannel();
                         if (channel == nullptr)
                         {
                             return;
                         }
                         ...
                         channelStates[channel->getName()] = value;
...
void SplitInput::themeChangedEvent()
{
    ...
    QPalette pointsPalette = this->ui_.channelPointsLabel->palette();
...
void SplitInput::openEmotePopup()
{
    ...
    this->emotePopup_->loadChannel(this->messageChannel());
    this->emotePopup_->show();
    this->emotePopup_->raise();
    this->emotePopup_->activateWindow();
}

ChannelPtr SplitInput::messageChannel() const
{
    if (this->replyTarget_ != nullptr)
    {
        if (auto channel = this->replyChannel_.lock())
        {
            return channel;
        }
    }

    return this->split_->getSelectedChannel();
}

void SplitInput::refreshMessageTargetState()
{
    this->updateEncryptionState();
    this->resetCommandCompletionSession();
    this->updateCompletionPopup();
...
void SplitInput::updateEncryptionState()
{
    if (this->ui_.encryptionToggleCheckbox == nullptr)
    {
        return;
    ...
    const auto channel = this->messageChannel();
    const auto channelStates =
        getSettings()->encryptionChannelStates.getValue();
    const QSignalBlocker blocker(this->ui_.encryptionToggleCheckbox);
    this->ui_.encryptionToggleCheckbox->setChecked(
        channel != nullptr && channelStates.value(channel->getName(), false));
}

void SplitInput::handleToggleEncryption()
{
    const auto c = this->messageChannel();
    if (c == nullptr)
    {
        return;
    }
...
void SplitInput::showCompletionPopup(const QString &text, CompletionKind kind)
{
    ...
    popup->updateCompletion(text, kind, this->messageChannel());
...
void SplitInput::editTextChanged()
{
    ...
    const auto channel = this->messageChannel();
    if (text.startsWith("/r ", Qt::CaseInsensitive) && channel != nullptr &&
    ...
    else
    {
        ...
        text = text.trimmed();
        text = app->getCommands()->execCommand(text, channel, true);
    }
...
void SplitInput::setReply(MessagePtr target, std::weak_ptr<Channel> channel)
{
    ...
    else if (this->enableInlineReplying_)
    {
        this->clearReplyTarget();
        return;
    }
    ...
    this->refreshMessageTargetState();
}
